...
class PyTorchDeepExplainer(Explainer):
    ...
    def gradient(self, idx, inputs, get_max_output=-1, mini_batch=200):
        self.model.zero_grad()
        # Added by Snow 19 Sep, 2019, 13 Mar 2020
        if get_max_output == -1:
            ...
            else: return [torch.autograd.grad(selected, x)[0].cpu().numpy() for x in X]
        else:
            X = inputs[0]
            nsamples = X.shape[0]
            all_grads = []

            def get_grad_mini_batch(mini_x):
                embeddings = self.model.embed(mini_x)
                del mini_x
                # retain the gradient in case it's all cleared out by the backward as embeddings are non-leaf
                embeddings.retain_grad()
                outputs = self.model(embeddings)


                selected = [val for val in outputs[:, idx]]
                grads = torch.sum(torch.autograd.grad(selected, embeddings, retain_graph=True)[0], 2)
            ...
            i = 0
            while nsamples - mini_batch * i > 0:
                mini_X = X[mini_batch * i: min(mini_batch * (i+1), nsamples), :]
                get_grad_mini_batch(mini_X)
                del mini_X
                i += 1
            del X
            ...
            # Added by Snow 19 Sep, 2019
            # else:
            #     # Added by Snow 27 Jul, 2020
            #     grads = torch.autograd.grad(selected, embeddings, retain_graph=True, create_graph=True)
            #     return grads
            # Added by Snow 27 Jul, 2020

    # Added by Snow 19 Sep, 2019
    def shap_values(self, X, ranked_outputs=None, output_rank_order="max", get_max_output=-1, mini_batch=200):
        # X ~ self.model_input
        # X_data ~ self.data
        ...
        # Added by Snow 19 Sep, 2019
        elif get_max_output >= 0:
            # self.num_outputs = 1
            self.multi_output = False
            model_output_ranks = torch.tensor([0]).unsqueeze(0)  # only requires to explain the max prediction
        # Added by Snow 19 Sep, 2019
        else:
        ...
        # Added by Snow 19 Sep, 2019
            ...
            for j in range(X[0].shape[0]):
                ...
                sample_phis = self.gradient(feature_ind, joint_x, get_max_output, mini_batch)
                ...
                if self.interim:
                    ...
                    for l in range(len(X)):
                        # Added by Snow 20 Sep, 2019
                        delta_x = (X[l][j: j + 1] - self.data[l])
                        delta_x = delta_x.type(torch.FloatTensor).to(X[l].device)
                        phis[l][j] = (torch.from_numpy(sample_phis[l][self.data[l].shape[0]:]).to(X[l].device) * delta_x).cpu().numpy().mean(0)
                        # Added by Snow 20 Sep, 2019
                del sample_phis
